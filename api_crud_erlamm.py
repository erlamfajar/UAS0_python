...
def write_db_api():
	...
	data = response.json()
	cursor = mydb.cursor()
	sql = '''INSERT INTO tbl_akademik_0479(id,nim,nama,jk,jurusan,alamat) VALUES (%s, %s, %s, %s, %s, %s )'''
	for i in data['data'] :
...
def tampil_data():
	...
	mycursor.execute("SELECT * FROM tbl_akademik_0479")
...
def tampil_limit():
	...
	mycursor = mydb.cursor()
	sql = ("SELECT * FROM tbl_akademik_0479 ")
...
def tampilkan_nim():
	...
	mycursor = mydb.cursor()
	sql = ("SELECT * FROM 'tbl_akademik_0479' WHERE nim=" )
